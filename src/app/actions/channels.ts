...
export async function getChannels(accessToken: string, workspaceId: string) {
  ...
  const userChannelIds = new Set(userChannelMemberships?.map(m => m.channel_id) || [])
  ...
  const accessibleChannels = allChannels?.filter(channel => 
    !channel.is_private || userChannelIds.has(channel.id)
  ) || []

  // Auto-add user to public channels they're not yet a member of
  const publicChannelsToJoin = accessibleChannels
    .filter(channel => !channel.is_private && !userChannelIds.has(channel.id))
...
export async function createChannel(accessToken: string, workspaceId: string, name: string, description: string = '', isPrivate: boolean = false) {
  const supabase = getSupabaseAdmin()
  ...
  if (memberError) {
    console.error('[createChannel] Failed to add creator as member:', memberError)
  }
  ...
  if (!isPrivate) {
    ...
    if (workspaceMembers && workspaceMembers.length > 0) {
      const channelMemberships = workspaceMembers
        .filter(wm => wm.user_id !== user.id) // Exclude creator (already added)
        .map(wm => ({
          channel_id: channel.id,
          user_id: wm.user_id,
        }))
